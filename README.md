...
- **Form (`app.py`)** — statisticians enter `trial_id`, `username`, and a list of questions. Each question has:
  ...
  - **Draft** — **Save draft** persists the current form (not a version) to `submissions/<DOI>__<username>/drafts/draft.json`, overwriting the previous draft. Re-enter the same DOI + username and click **Load draft** to restore it. Drafts are excluded from version listings and the admin console.
